...
def EEGX_Pretraining(config, Data):
    ...
    save_path = os.path.join(config['save_dir'], config['problem'] + '_finetuned_model_{}.pth'.format('last'))
...
class EEGX_SelfSupervised_Trainer(object):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        if (epoch_num + 1) % 5 == 0:
            ...
            y_hat = clf.predict(test_repr.cpu().detach().numpy())
            acc_test = accuracy_score(test_labels.cpu().detach().numpy(), y_hat)
            # plot_tSNE(test_repr.cpu().detach().numpy(), test_labels.cpu().detach().numpy(), epoch_num)
            print('Test_acc:', acc_test)
...
def plot_tSNE(data, labels, epoch_num):
    ...
    plt.plot(class_0_data[:, 0], class_0_data[:, 1], 'bo', label='Low workload')
    plt.plot(class_1_data[:, 0], class_1_data[:, 1], 'ro', label='High workload')
    # plt.scatter(class_0_data[:, 0], class_0_data[:, 1], c='blue', label='low workload', alpha=0.6, s=50)
    # plt.scatter(class_1_data[:, 0], class_1_data[:, 1], c='red', label='high workload', alpha=0.6, s=50)
    # plt.scatter(class_1_data[:, 0], class_1_data[:, 1], c='#FFB84D', label='High workload', alpha=0.6, s=50)
    # plt.scatter(class_0_data[:, 0], class_0_data[:, 1], c='#4C9EE2', label='Low workload', alpha=0.4, s=50)
    plt.legend(fontsize='large', loc='lower left')
    plt.grid(False)  # Remove grid
    plt.axis(False)
    plt.savefig(f'SSL_epoch_{epoch_num}.pdf', bbox_inches='tight', format='pdf')
    # plt.show()
