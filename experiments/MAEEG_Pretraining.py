...
def MAEEG_Pretraining(config, Data):
    ...
    SS_trainer = MAEEG_SelfSupervised_Trainer(Encoder, pre_train_loader, train_loader, test_loader, config, l2_reg=0, print_conf_mat=False)
...
class MAEEG_SelfSupervised_Trainer(object):
    def __init__( self, model, pre_train_loader, train_loader, test_loader, config, optimizer=None, l2_reg=None, print_interval=10,
        ...
        super(MAEEG_SelfSupervised_Trainer, self).__init__()
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.pre_train_loader):
            X, _, _ = batch
            target_prediction = self.model.pretrain_forward(X.to(self.device))
            total_loss = F.mse_loss(X.to(self.device), target_prediction)
        ...
        if (epoch_num + 1) % 5 == 0:
            ...
            print(f'{int(epoch_num)}, {acc_test}, {epoch_loss}', file=result_file)
            result_file.close()
